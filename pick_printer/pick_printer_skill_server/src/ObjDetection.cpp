...
geometry_msgs::Point ObjDetec::pointsToPose(geometry_msgs::Point p1,geometry_msgs::Point p2){
    ...
    ponto.y = atan(sqrt( pow((zp-zc),2)+pow((yp-yc),2))/(xp-xc));
    ponto.z = atan(sqrt(pow((xp-xc),2)+pow((zp-zc),2))/(yp-yc));
    /*
    cout << "Angulo x:" << ponto.x*pid << endl;
    cout << "Angulo y:" << ponto.y*pid << endl;
  ...
  if (!opts.no_images) {
   ...
   for (size_t i=0; i<detections.size(); ++i) {
      ...
      if (d.id == id){
        ...
        cv::circle( src, d.p[3], 4, cv::Scalar(255,255,255), -1, 8, 0 );
        cv::circle( src, center, 4, cv::Scalar(0,0,0), -1, 8, 0 );
        ObjDetec::angle=atan2(d.p[1].y - d.p[0].y, d.p[1].x - d.p[0].x);

        
        auto c1=ObjDetec::pixelTo3DPoint(cam.get_cloud(),d.p[0].x,d.p[0].y);
        auto c2=ObjDetec::pixelTo3DPoint(cam.get_cloud(),d.p[1].x,d.p[1].y);
        auto c3=ObjDetec::pixelTo3DPoint(cam.get_cloud(),d.p[2].x,d.p[2].y);
        ...
        cout << "Rotacao: " << ObjDetec::angle*180/M_PI << endl;
